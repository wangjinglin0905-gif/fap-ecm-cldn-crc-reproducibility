...
set.seed(20260807)
PROJ <- '.'
OUT  <- file.path(PROJ, 'output', 'review_r13_senescence_expansion')
...
senmayo <- read_genes(file.path(PROJ, 'data', 'senmayo', 'SenMayo_125_genes.txt'))
cellage <- read_genes(file.path(PROJ, 'data', 'senmayo', 'CellAge_inducing_genes.txt'))
...
expr380 <- as.matrix(read.table(gzfile('data/TCGA/TCGA_COADREAD_expression.txt.gz'),
...
m592 <- NULL
f592 <- file.path(PROJ, 'data', 'tcga_592', 'TCGA592_target_genes_expression.csv')
if (file.exists(f592)) {
...
gse_pheno <- read.csv(file.path(PROJ, 'data', 'GSE39582', 'GSE39582_pheno.csv'),
                      check.names = FALSE, stringsAsFactors = FALSE)
target <- unique(c(FAP13, matrix4, receptor2, senmayo, SASP25, colonSASP7, 'MKI67','CDKN2A','CDKN2B'))
...
# 仅保留肿瘤样本（pheno 匹配）
m_gse <- m_gse[rownames(m_gse) %in% gse_pheno$sample, , drop = FALSE]
cat('GSE39582:', nrow(m_gse), 'samples; genes:', ncol(m_gse), '\n')
cat('GSE target hit:', sum(target %in% colnames(m_gse)), '/', length(target), '\n')

# Clinical (TCGA)
clin <- read.delim(file.path(PROJ, 'data', 'tcga_clinical', 'COADREAD_clinicalMatrix.txt'),
                   check.names = FALSE, stringsAsFactors = FALSE)
...
mcp <- read.csv('legacy_external/fap_jcmm_reproducibility/results/L4_immune/mcpcounter_tumor_scores.csv',
                check.names = FALSE, stringsAsFactors = FALSE)
mcp_fib <- data.frame(sample = names(mcp)[-1], caf = as.numeric(unlist(mcp[1, -1])))
epic <- read.csv('legacy_external/fap_jcmm_reproducibility/results/L4_immune/epic_tumor_cell_fractions.csv',
                 check.names = FALSE, stringsAsFactors = FALSE)
...
sig_names <- c(SenMayo = 'SenMayo', SASP25 = 'SASP25', colonSASP = 'colonSASP', CellAge = 'CellAge')
multi <- list()
for (sn in names(sig_names)) {
...
if ('os_event' %in% colnames(clin)) {
  ...
  for (g in c('FAP13','matrix4','SenMayo','SASP25','colonSASP')) {
    ...
    cox_out[[paste0('TCGA_', g)]] <- c(HR = exp(s[1]), ci_low = exp(s[1]-1.96*s[3]),
                                       ci_high = exp(s[1]+1.96*s[3]), P = s[5], n = nrow(surv_df))
...
if ('os_event' %in% colnames(gse_pheno) && 'os_delay_months' %in% colnames(gse_pheno)) {
  ...
  for (g in c('FAP13','matrix4','SenMayo','SASP25','colonSASP')) {
    ...
    s <- summary(m)$coefficients[1, ]
    cox_out[[paste0('GSE_', g)]] <- c(HR = exp(s[1]), ci_low = exp(s[1]-1.96*s[3]),
                                      ci_high = exp(s[1]+1.96*s[3]), P = s[5], n = nrow(surv_gse))
